...
//#include "stdafx.h"	// RG_ONLY: not required?
#include <iostream>
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
...
#ifdef	WIN32
	#ifdef	_DEBUG
	// RG_ONLY: use debugbreak because compiler complains about using asm
...
static	void	doCleanupLogOnFirstRun()
{
	...
	{
		_unlink(memoryLogFile); // RG_ONLY: unlink -> _unlink (because unlink is deprecated)
...
static	sAllocUnit	*findAllocUnit(const void *reportedAddress)
{
	...
	size_t hashIndex = (((size_t)reportedAddress) >> 4) & (hashSize - 1); // RG_ONLY: size_t
...
static	void	dumpAllocations(FILE *fp)
{
	...
	for (unsigned int i = 0; i < hashSize; i++)
	{
		...
		{
			// RG_ONLY: size_t
			fprintf(fp, "%06d 0x%016zX 0x%08zX 0x%016zX 0x%08zX 0x%08X %-8s    %c       %c    %s\r\n",
...
void	*operator new(size_t reportedSize)
{
	...
	for(;;)
	{
		...
		std::new_handler	nh = std::set_new_handler(0); // RG_ONLY: std::new_handler
...
void	*m_allocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int allocationType, const size_t reportedSize)
{
	try
	{
		...
		size_t	hashIndex = ((size_t)(au->reportedAddress) >> 4) & (hashSize - 1); // RG_ONLY: size_t
		...
		if (alwaysLogAll) log("[+] ---->             addr 0x%016zX", (size_t)(au->reportedAddress)); // RG_ONLY: size_t
...
void	*m_reallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int reallocationType, const size_t reportedSize, void *reportedAddress)
{
	try
	{
		...
		if (oldReportedAddress != au->reportedAddress)
		{
			...
			{
				size_t	hashIndex = ((size_t)(oldReportedAddress) >> 4) & (hashSize - 1); // RG_ONLY: size_t
			...
			hashIndex = ((size_t)(au->reportedAddress) >> 4) & (hashSize - 1); // RG_ONLY: size_t
			if (hashTable[hashIndex]) hashTable[hashIndex]->prev = au;
			au->next = hashTable[hashIndex];
			au->prev = NULL;
			hashTable[hashIndex] = au;
...
void	m_deallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int deallocationType, const void *reportedAddress)
{
	try
	{
		...
		if (alwaysLogAll) log("[-] ----- %8s of addr 0x%016zX           by %s", allocationTypes[deallocationType], (size_t)(const_cast<void *>(reportedAddress)), ownerString(sourceFile, sourceLine, sourceFunc)); // RG_ONLY: size_t
